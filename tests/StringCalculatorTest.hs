...
main = hspec $ do
  describe "summation of delimited string of numbers:" $ do
    context "operations around delimiters:" $ do
      ...
      it "one number results in int of said numbers" $ do
        add "1" `shouldBe` 1

      it "two numbers results in int sum of said numbers" $ do
        add "1,2" `shouldBe` 3

      it "n numbers results in int sum of said numbers" $ do
        add "1,2,3" `shouldBe` 6

      it "use newline along with comma as delimiters" $ do
        add "1,2\n3" `shouldBe` 6
